...
private const val DEFAULT_TIMER_PERIOD: Long = 100L
...
class TicketsSearchViewModel(
    ...
    private val fromCityMarkerLiveData: MutableLiveData<MapModel.CityMarker> = MutableLiveData()
    private val toCityMarkerLiveData: MutableLiveData<MapModel.CityMarker> = MutableLiveData()
    ...
    init {
        ...
        fromCityMarkerLiveData.value = MapModel.CityMarker(
            point = locationToLatLngMapper.map(fromCity.location),
            text = fromCity.iata.firstOrNull() ?: DEFAULT_IATA
        )
        toCityMarkerLiveData.value = MapModel.CityMarker(
    ...
    fun getFromCityMarkerLiveData(): LiveData<MapModel.CityMarker> = fromCityMarkerLiveData

    fun getToCityMarkerLiveData(): LiveData<MapModel.CityMarker> = toCityMarkerLiveData
